...
<body>
	...
	<div style="width: 100%;">
		...
		<div style="width: 84%; float: right; margin-top: 0px;">
			...
			<div>
				<span>当前位置:不及格统计</span>
			</div>
			<div style="margin-bottom: 10px;">
				<center>
					<form action="StuAny.jsp" method="post">
			...
			<table class="stu_info_table" id="stu_info_table" cellspacing="0px"
				...
				<%
					ArrayList<FailedByClassAndCourse> stuList = FailedByClassAndCourseDao.classFailed(cl);
					
					if (stuList.isEmpty()) {
						out.println("暂无消息");
					}
				
					for (FailedByClassAndCourse ss : stuList) {
						ArrayList<PersonalFailedClasses> stuCourseFailedList = PersonalFailedClassesDao.failedPass(ss.getStuNum());
				%>
				<tr>
					
						<td><%=ss.getStuNum()%></td>
						<td><a href = "show_detail_fail.jsp?stu_num=<%=ss.getStuNum()%>"><%=ss.getName()%></a></td>
						<td><%=stuCourseFailedList.size()%></td>
				</tr>
				<% } %>
			</table>
		</div>
