	...
	<div style="width: 100%;">
		...
		<div style="width: 84%; float: right; margin-top: 0px;">
			...
			<div style="margin-bottom: 10px;">
				<center>
					<form action="StuClassCount.jsp" method="post">
			...
			<table class="stu_info_table" id="stu_info_table" cellspacing="0px"
				...
				<%
					ArrayList<ClassTotalScore> stuList = ClassScoreDao.queryScoreByClass(cl+"");
					if (stuList.isEmpty()) {
						out.println("暂无消息");
					}
				
					for (ClassTotalScore ss : stuList) {
				%>
				<tr>
					
						<td><%=ss.getStuNum()%></td>
						<td><a href = "show_detail.jsp?stu_num=<%=ss.getStuNum()%>"><%=ss.getStuName()%></a></td>
						<td><%=ss.getTotal() %></td>
						<td><%=ss.getAverage() %></td>
